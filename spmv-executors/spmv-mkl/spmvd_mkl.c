...
for( t = 0; t < REPS; ++t ) {
    ...
    mkl_sparse_d_mv( SPARSE_OPERATION_NON_TRANSPOSE, // op(A) = A
                     1.0, // alpha = 1
...
int main(int argc, char** argv)
{
  ...
  sparse_matrix_t M;
  mkl_sparse_d_create_csr( &M, SPARSE_INDEX_BASE_ZERO, nrow_mkl, ncol_mkl, indptr_mkl, &(indptr_mkl[1]), indices_mkl, A );
    struct matrix_descr descr;
    descr.type = SPARSE_MATRIX_TYPE_GENERAL;
