...
reg_byte = [ "%r8b", "%r9b", "%r10b", "%r11b", "%r12b", "%r13b", "%r14b", "%r15b", "%sil", "%dil"  ]
reg_double = [ "%r8d", "%r9d", "%r10d", "%r11d", "%r12d", "%r13d", "%r14d", "%r15d" ]
reg_numbered = [ "%r8" ]
reg_32 = [ "%rdi", "%rdx", "%rsi", "%rbp", "%rax", "%rip", "%rbx", "%rcx", "%eax", "%ecx", "%edx", "%ebx", "%esi", "%edi" ]
reg_64 = [ "%r9", "%r10", "%r11", "%r12", "%r13", "%r14", "%r15", "%rsp" ]
reg_sse= [ "%xmm0", "%xmm1", "%xmm2", "%xmm3", "%xmm4", "%xmm5", "%xmm6", "%xmm7", "%xmm8", "%xmm9", "%xmm10", "%xmm11", "%xmm12", "%xmm13", "%xmm14", "%xmm15", "%xmm16", "%xmm17", "%xmm18", "%xmm19", "%xmm20", "%xmm21", "%xmm22", "%xmm23", "%xmm24", "%xmm25", "%xmm26", "%xmm27", "%xmm28", "%xmm29", "%xmm30", "%xmm31" ]
reg_avx= [ "%ymm0", "%ymm1", "%ymm2", "%ymm3", "%ymm4", "%ymm5", "%ymm6", "%ymm7", "%ymm8", "%ymm9", "%ymm10", "%ymm11", "%ymm12", "%ymm13", "%ymm14", "%ymm15" ]
...
def asm_prefetch( path, outf=sys.stdout ):
    ...
    for l in f:
        ...
        if words[0].startswith( ".align" ):
            ...
            offset = int(ceil(float(offset)/alignment_factor) * alignment_factor)
            ...
            if words[0][:-1] in noprefetch_region_labels:
                if words[0][:-1] not in jump_idx:  
                    ...
                    for idx in jump_idx[label]:
                        jump_distance = target_addr - global_addr - offsets[idx]
                        if jump_distance < -128 or jump_distance >= 128:
                            ...
                            for x in xrange( idx, len(offsets) ):
                                ...
                                if x in alignment_idx:
                                    ...
                                    new_offset = int(ceil(float(offsets[x-1])/alignment_idx[x]) * alignment_idx[x])
...
#                                        offsets[y] += delta
        ...
        if words[0] in "vmovsd":
            if words[1][:-1] in reg_sse:
                if words[2][:-1] in reg_sse:
                    if words[3] in reg_sse:
                        offset += 4
                        higher = 0
                        m = re.match( r"%xmm(\d+)", words[1][:-1] )
                        if int(m.group(1)) > 7: higher += 1
                        m = re.match( r"%xmm(\d+)", words[3] )
                        if int(m.group(1)) > 7 : higher += 1
                        if higher > 1: offset += 1
                        continue
                else:
                    m = re.match( r"(.+)?\(([^,]+)\)", words[2] )
                    if m is not None:
            ...
            if m.group(1) is None: 
                ...
                if m.group(2) in reg_32:
                    if m.group(3) in reg_32:
                        if m.group(4) is None: raise NotImplementedError
                        else:
        ...
        if words[0] in "incl":
            ...
            m = re.match( r"(.+)?\(([^,]+)\)", words[1] )
            if m.group(1) is None: raise NotImplementedError
            elif int(m.group(1)) >= -128 and int(m.group(1)) < 128:
                if m.group(2) in reg_64:
                    offset += 4
                    continue
            ...
            if words[1][:-1] in reg_avx or words[1][:-1] in reg_sse:
                if words[2][:-1] in reg_avx or words[2][:-1] in reg_sse:
                    if words[3] in reg_avx or words[3] in reg_sse:
                        offset += 4
                        m = re.match( r"%(x|y)mm(\d+)", words[1][:-1] )
                        # If destination register is larger than 7, +1 byte
                        if int(m.group(2)) > 7: offset += 1
                        continue
        ...
        if words[0] in "vmovaps":
            if words[1][:-1] in reg_sse:
                if words[2] in reg_sse:
                   ...
                   m = re.match( r"%xmm(\d+)", words[1][:-1] )
                   if int(m.group(1)) > 7: higher += 1
                   m = re.match( r"%xmm(\d+)", words[2] )
                   if int(m.group(1)) > 7 : higher+= 1
                   if higher == 2: offset += 1
                   continue
        ...
        if words[0] in "vextractf128":
            ...
            if int( m.group(1) ) < 2:
                if words[2][:-1] in reg_avx:
                    ...
                    m = re.match( r"(.+)?\((.+)\)", words[3] )
                    if m.group(1) is None:
                        if m.group(2) in reg_32: raise NotImplementedError(l)
                        elif m.group(2) in reg_64:
                            offset += 7
                            continue
                    elif int(m.group(1)) >= -128 and int(m.group(1)) < 128:
        ...
        if words[0] in ["subq","addq"]:
            ...
            m = re.match( r"\$(.+)", words[1][:-1] )
            if m is not None:
                if m.group(1) is None: raise NotImplementedError(l)
                elif int(m.group(1)) >= -128 and int(m.group(1)) < 128:
                    if words[2] in reg_32 or words[2] in reg_64 or words[2] in reg_numbered:
                        offset += 4
                        continue
        ...
        if words[0] in "vpermpd":
            ...
            if m.group(1) is None: raise NotImplementedError(l)
                ...
                m = re.match( r"(.+)?\(([^,]+)?,([^,]+)(,([^,]+))?\)", words[2][:-1] )
                if m.group(1) is None: raise NotImplementedError
                elif int(m.group(1)) >= -128 and int(m.group(1)) < 128: raise NotImplementedError
                else:
                    if m.group(2) in reg_32:
                        if m.group(3) in reg_64:
                            if int(m.group(5)) >= -128 and int(m.group(5)) < 128:
                                    ...
                                    if int(m.group(5)) >= -128 and int(m.group(5)) < 128:
                                        if words[3][:-1] in reg_avx:
                                            if words[4] in reg_avx:
                                                offset += 11
                                                continue
            else: raise NotImplementedError(l)
        ...
        if words[0] in "vmovapd":
            if words[1][:-1] in reg_sse or words[1][:-1] in reg_avx:
                if words[2] in reg_sse or words[2] in reg_avx:
                    ...
                    higher = 0
                    m = re.match( r"%(x|y)mm(\d+)", words[1][:-1] )
                    if int(m.group(2)) > 7: higher += 1
                    m = re.match( r"%(x|y)mm(\d+)", words[2] )
                    if int(m.group(2)) > 7: higher += 1
                    if higher == 2: offset += 1
                    continue
        ...
        if words[0] in "vmovups":
            ...
            if words[1].startswith( ".L" ):
                ...
                words[1] = re.sub( r".*(\(.*\))", r"1024\\1", words[1] )
        ...
        if words[0] in "vmovdqu":
            if words[1][:-1] in reg_avx:
                m = re.match( r"(.+)?\((.+)\)", words[2] )
                if m.group(1) is None:
                    if m.group(2) in reg_32: raise NotImplementedException(l)
                    elif m.group(2) in reg_64:
                        offset += 5
                        continue
                elif int(m.group(1)) >= -128 and int(m.group(1)) < 128:
...
#        line_no += 1
        if fragment is None:
            m = re.match( r"([0-9a-fA-F]+) <kernel_spmv_fragment_(\d+)>:", l )
            if not m is None:
                start_addr = int(m.group(1),16)
                fragment   = int(m.group(2))
        else:
            m = re.match( r"\s+([0-9a-fA-F]+):.+retq\s*$", l )
            if m is not None:
                match = "ret"
...
#                    fline_no += 1
                    if not fl_fragment:
                        m = re.match( r"<\+\d+>\s+kernel_spmv_fragment_%d:$" % fragment, fl )
                        if not m is None: fl_fragment = True
                    else:
                        m = re.match( r"<\+(\d+)>\s+ret\s+", fl )
                        if not m is None:
                            match_addr = int(m.group(1))
#                            if not match_addr == end_addr: raise ValueError()
                            rel_err = abs( float(match_addr - end_addr) / min(match_addr,end_addr) )
                            if rel_err > max_err: raise ValueError()
                            print( "Fragment %d: match == +%s (+/- %.2f%%)" % (fragment, hex(match_addr), rel_err*100) )
                            fragment = None
                            break
                else:
                    raise ValueError()
            else:
                    m = re.match( r"(\d+) <kernel_spmv_fragment_(\d+)>:", l )
                    if m is not None:
                        ipdb.set_trace()
...
def smart_insertion( asm_path, dump_path, outf=sys.stdout ):
    fasm = open( asm_path )
    fdump = open( dump_path )

    # Turn it around: we need to traverse the ASM, printing everything as we go.
    # Upon finding a new fragment, open the dumped code with addresses and traverse both the ASM and the dumped code
    # at the same time, inserting prefetches where appropriate

    fragment = None
    prefetches = []
    for pos,l in enumerate(fasm):
        if fragment is None:
            m = re.match( r"kernel_spmv_fragment_(\d+):$", l ) 
            if m is not None:
                ...
                while not in_position:
                    try: fl = fdump.readline()
                    except StopIteration: break
                    m = re.match( r"([0-9a-fA-F]+) <kernel_spmv_fragment_%d>:" % fragment, fl )
                    if m is not None: 
                        in_position=True
            ...
            while curr_addr - prev_addr < 64:
                try: fl = fdump.readline()
                except StopIteration: break
                m = re.match( r"\s+([0-9a-fA-F]+):\t(.*)\t(.*)", fl )
                if m is not None:
                    if m.group(3).startswith( "ret" ):
                        fragment = None
                        break
                    instr_cursor += 1
                    curr_addr = int(m.group(1),16)
            if l.strip().startswith("#"): pass
            elif l.strip().startswith("."): pass
            elif l.strip().startswith( "ret" ): 
                fragment = None
            elif l.strip() == "": pass
            else: instr_cursor -= 1
            if instr_cursor == 0:
                print( "\tprefetcht1 0x1000(%rip)", file=outf )
                prev_addr = curr_addr - 7
            if instr_cursor < 0: raise ValueError
        print(l,end="",file=outf)
...
if __name__ == "__main__":
    try:
        ...
        print( "Usage: fix_asm <asm_path>" )
        sys.exit(0)

    smart_insertion( path, dumpfile, outf=outf )
