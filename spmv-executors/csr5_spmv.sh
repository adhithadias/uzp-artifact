...
if [[ "$FLOAT_DATA_TYPE" != "double" ]]; then
    echo -e "${ERROR_TAG} Invalid FLOAT_DATA_TYPE: $FLOAT_DATA_TYPE. Must be 'double' (float not supported by this baseline)."
    exit 1
else
...
make -C spmv-csr5/CSR5_avx2 -B VALUE_TYPE=${FLOAT_DATA_TYPE} NUM_RUN=${KERNEL_REPS}

RUNCOUNT=10
i=0; while [[ $i -lt RUNCOUNT ]]; do
    OMP_NUM_THREADS=${OMP_NUM_THREADS} numactl -C${PIN_CORES} hugectl --heap ./spmv-csr5/CSR5_avx2/${CMD} "$TEMP_DIRECTORY/${MATRIX_NAME}/${MATRIX_NAME}.mtx"
    i=$((i+1));
done
