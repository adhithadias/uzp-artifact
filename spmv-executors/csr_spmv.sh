...
make -C spmv-csr -B ${CMD} KERNEL_REPS=${KERNEL_REPS}

RUNCOUNT=10
i=0; while [[ $i -lt RUNCOUNT ]]; do
    OMP_NUM_THREADS=${OMP_NUM_THREADS} numactl -C${PIN_CORES} hugectl --heap ./spmv-csr/${CMD} "$TEMP_DIRECTORY/${MATRIX_NAME}/${MATRIX_NAME}.rb"
    i=$((i+1));
done
